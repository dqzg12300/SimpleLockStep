...
function mysqldb:findOneOrder(cname, selector,ordername,field_selector)
    local table_desc = self.table_desc
    local db = self.db
    ...
    local sql = string.format("select %s from %s where %s order by %s desc limit 1 ", field_selector_str, cname, selector_str,ordername)
    log.debug("sql: %s", sql)
    local ret = db:query(sql)
...
function mysqldb:insert(cname, data)
    ...
    local sql = string.format("insert ignore into %s(%s)values(%s)", cname, field_str, value_str)
    log.debug("sql: %s", sql)
    local ret = db:query(sql)
    log.debug("ret: %s", tool.dump(ret))
    return ret
